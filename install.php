...
require_once 'uD_config.php';//loads config
    ...
    try {//t1
			...
			$e=$dbh->query("SELECT 1 FROM INFORMATION_SCHEMA.SCHEMA_PRIVILEGES WHERE GRANTEE = '''$user''@''$host''';")->fetchAll();
			$user_exists=$e[0];
			
			if(!$user_exists)
			{
				echo "Creating User $user : $pass <br>";
				$dbh->exec("CREATE USER `$user`@`$host` IDENTIFIED BY '$pass';
							GRANT ALL ON $db.* TO `$user`@`$host` WITH GRANT OPTION;
							
							FLUSH PRIVILEGES;") or  trigger_error('Creating User Failed: ' . mysql_error(), E_USER_ERROR);
     ...
     try{//t2
		...
		/*$dbhh->exec("SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
					SET time_zone = '+00:00';
